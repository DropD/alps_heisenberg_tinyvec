...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        if (boost::filesystem::extension(options.input_file) == ".xml") {
            ...
            //~ old_parameters = alps::Parameters(alps::hdf5::archive(options.input_file)["/parameters"]);
        }
        else {
            parameters = alps::parameters_type<heisenberg_sim>::type(options.input_file);
            //~ std::ifstream fin(options.input_file.c_str(), std::ifstream::in);
            //~ std::istream& si = fi;
            //~ alps::Parameters old_parameters(fin);
        }
