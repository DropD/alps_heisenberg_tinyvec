...
#define _AVX_CONST(Name, Val)                                            \
	static const ALIGN16_BEG double _pd256_##Name[4] ALIGN16_END = { (const double)(Val), (const double)(Val), (const double)(Val), (const double)(Val) }
#define _AVX_CONST_TYPE(Name, Type, Val)                                 \
	static const ALIGN16_BEG Type _pd256_##Name[4] ALIGN16_END = { Val, Val, Val, Val }
...
_AVX_CONST_TYPE(sign_mask, long long, 0x8000000000000000LL);
_AVX_CONST_TYPE(inv_sign_mask, long long, ~0x8000000000000000LL);

_AVX_CONST(minus_cephes_DP1, -0.78515625);
_AVX_CONST(minus_cephes_DP2, -2.4187564849853515625e-4);
_AVX_CONST(minus_cephes_DP3, -3.77489497744594108e-8);
_AVX_CONST(sincof_p0, -1.9515295891E-4);
_AVX_CONST(sincof_p1,  8.3321608736E-3);
_AVX_CONST(sincof_p2, -1.6666654611E-1);
_AVX_CONST(coscof_p0,  2.443315711809948E-005);
_AVX_CONST(coscof_p1, -1.388731625493765E-003);
_AVX_CONST(coscof_p2,  4.166664568298827E-002);
_AVX_CONST(cephes_FOPI, 1.27323954473516); // 4 / M_PI
...
{ v4sd xmm1, xmm2, sign_bit_sin, y, y2, z, swap_sign_bit_sin, poly_mask;
  v4sd sign_bit_cos;
#ifdef USE_SSE2
  v4si emm2;
#else
...
//   emm2 = _mm_add_epi32(emm2, *(v4si*)_pi32_1);
//   emm2 = _mm_and_si128(emm2, *(v4si*)_pi32_inv1);
	emm2 = _mm_and_si128( _mm_add_epi32( _mm256_cvttpd_epi32(y), *(v4si*)_pi32_1 ), *(v4si*)_pi32_inv1 );
	y = _mm256_cvtepi32_pd(emm2);

  /* get the swap sign flag for the sine */
//   emm0 = _mm_and_si128(emm2, *(v4si*)_pi32_4);
//   emm0 = _mm_slli_epi32(emm0, 29);
//   swap_sign_bit_sin = _mm256_castsi128_pd(emm0);
    v4si tmp0 = _mm_slli_epi32( _mm_and_si128(emm2, *(v4si*)_pi32_4), 29);
    long * tmp0_v = (long *)(&tmp0);
    v4sd tmp1 = _mm256_set_epi64x(tmp0_v[0], tmp0_v[1], tmp0_v[2], tmp0_v[3]);
	swap_sign_bit_sin = _mm256_castsi256_pd(tmp1);

  /* get the polynom selection mask for the sine*/
//   emm2 = _mm_and_si128(emm2, *(v4si*)_pi32_2);
//   emm2 = _mm_cmpeq_epi32(emm2, _MM_SETZERO_SI128());
//   poly_mask = _mm256_castsi128_pd(emm2);
    tmp0 = _mm_cmpeq_epi32( _mm_and_si128(emm2, *(v4si*)_pi32_2), _MM_SETZERO_SI128());
    tmp0_v = (long *)(&tmp0);
    tmp1 = _mm256_set_epi64x(tmp0_v[0], tmp0_v[1], tmp0_v[2], tmp0_v[3]);
	poly_mask = _mm256_castsi256_pd(tmp1);
#else
  { v4sd xmm3 = _mm256_setzero_pd();
	  /* store the integer part of y in mm2:mm3 */
	  xmm3 = _mm256_movehl_pd(xmm3, y);
	  mm2 = _mm256_cvttpd_pi32(y);
	  mm3 = _mm256_cvttpd_pi32(xmm3);
  ...
  /* The magic pass: "Extended precision modular arithmetic"
     x = ((x - y * DP1) - y * DP2) - y * DP3; */
#ifdef __GNUC__
	x += y * ( *(v4sd*)_pd256_minus_cephes_DP1 + *(v4sd*)_pd256_minus_cephes_DP2 + *(v4sd*)_pd256_minus_cephes_DP3 );
#else
//   xmm1 = *(v4sd*)_ps_minus_cephes_DP1;
//   xmm2 = *(v4sd*)_ps_minus_cephes_DP2;
//   xmm3 = *(v4sd*)_ps_minus_cephes_DP3;
...
//   x = _mm256_add_pd(x, xmm3);
  	x = _mm256_add_pd( x, _mm256_mul_pd( y, _mm256_add_pd( _mm256_add_pd(*(v4sd*)_pd256_minus_cephes_DP1, *(v4sd*)_pd256_minus_cephes_DP2),
										*(v4sd*)_pd256_minus_cephes_DP3 ) ) );
#endif
...
//   emm4 = _mm_andnot_si128(emm4, *(v4si*)_pi32_4);
//   emm4 = _mm_slli_epi32(emm4, 29);
//   sign_bit_cos = _mm256_castsi128_pd(emm4);
    tmp0 = _mm_slli_epi32( _mm_andnot_si128( _mm_sub_epi32(emm2, *(v4si*)_pi32_2), *(v4si*)_pi32_4), 29 );
    tmp0_v = (long *)(&tmp0);
    tmp1 = _mm256_set_epi64x(tmp0_v[0], tmp0_v[1], tmp0_v[2], tmp0_v[3]);
	sign_bit_cos = _mm256_castsi256_pd(tmp1);
#else
  /* get the sign flag for the cosine */
  mm4 = _mm_sub_pi32(mm4, *(v2si*)_pi32_2);
  mm5 = _mm_sub_pi32(mm5, *(v2si*)_pi32_2);
...
#ifdef __GNUC__
  z = x * x;
  y = ( ( ( (*(v4sd*)_pd256_coscof_p0) * z + *(v4sd*)_pd256_coscof_p1 ) * z + *(v4sd*)_pd256_coscof_p2 ) * z
  		- *(v4sd*)_pd256_0p5 ) * z + *(v4sd*)_pd256_1;
#else
  z = _mm256_mul_pd(x,x);
//   y = *(v4sd*)_ps_coscof_p0;
//
//   y = _mm256_mul_pd(y, z);
//   y = _mm256_add_pd(y, *(v4sd*)_ps_coscof_p1);
//   y = _mm256_mul_pd(y, z);
//   y = _mm256_add_pd(y, *(v4sd*)_ps_coscof_p2);
//   y = _mm256_mul_pd(y, z);
//   y = _mm256_mul_pd(y, z);
//   tmp = _mm256_mul_pd(z, *(v4sd*)_ps_0p5);
//   y = _mm256_sub_pd(y, tmp);
//   y = _mm256_add_pd(y, *(v4sd*)_ps_1);
	y = _mm256_add_pd(
		_mm256_mul_pd(
			_mm256_sub_pd(
				_mm256_mul_pd(
					_mm256_add_pd(
						_mm256_mul_pd(
							_mm256_add_pd(
								_mm256_mul_pd(*(v4sd*)_pd256_coscof_p0, z),
								*(v4sd*)_pd256_coscof_p1 ),
							z ),
						*(v4sd*)_pd256_coscof_p2 ),
					z ),
				*(v4sd*)_pd256_0p5 ),
			z ),
		*(v4sd*)_pd256_1 );
...
#ifdef __GNUC__
	y2 = ( ( ( ( ((*(v4sd*)_pd256_sincof_p0) * z ) + *(v4sd*)_pd256_sincof_p1 ) * z ) + *(v4sd*)_pd256_sincof_p2 ) * z
		+ *(v4sd*)_pd256_1 ) * x;
#else
//   y2 = *(v4sd*)_ps_sincof_p0;
//   y2 = _mm256_mul_pd(y2, z);
//   y2 = _mm256_add_pd(y2, *(v4sd*)_ps_sincof_p1);
//   y2 = _mm256_mul_pd(y2, z);
//   y2 = _mm256_add_pd(y2, *(v4sd*)_ps_sincof_p2);
//   y2 = _mm256_mul_pd(y2, z);
		...
		_mm256_add_pd(
			_mm256_mul_pd(
				_mm256_add_pd(
					_mm256_mul_pd(
						_mm256_add_pd(
							_mm256_mul_pd(*(v4sd*)_pd256_sincof_p0, z ),
							*(v4sd*)_pd256_sincof_p1 ),
						z ),
					*(v4sd*)_pd256_sincof_p2 ),
				z ),
			*(v4sd*)_pd256_1 ),
		x );
#endif
...
#if defined(__GNUC__0) && !defined(__MINGW32__)
	  // less precise results
	  xmm1 = _mm256_andnot_pd( poly_mask, y) + (poly_mask & y2);
	  xmm2 = y + y2 - xmm1;
	  /* update the sign */
	  *s = xmm1 | sign_bit_sin;
	  *c = xmm2 | sign_bit_cos;
