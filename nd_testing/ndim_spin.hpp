...
class ALPS_DECL ndim_spin_sim : public alps::mcbase {
    ...
    public:
        typedef tinyvector<N, S> spintype;
...
template<int N, class S>
ndim_spin_sim<N, S>::ndim_spin_sim(parameters_type const & parms, std::size_t seed_offset)
...
void ndim_spin_sim<N, S>::update() {
    for (int j = 0; j < lattice.num_sites(); ++j) {
        ...
        /* sum all neighbors */
        typename ndim_spin_sim<N, S>::spintype nn_sum;
        nn_sum.initialize(0);
        ...
        typename ndim_spin_sim<N, S>::spintype new_spin = random_spin();
...
template <int N, class S>
void ndim_spin_sim<N, S>::measure() {
    sweeps++;
    if (sweeps > thermalization_sweeps) {
        typename ndim_spin_sim<N, S>::spintype tmag;
        tmag.initialize(0);
        ...
        measurements["Magnetization"] << spintype::vector(tmag);
...
template <int N, class S>
double ndim_spin_sim<N, S>::fraction_completed() const {
    return (sweeps < thermalization_sweeps ? 0. : ( sweeps - thermalization_sweeps ) / double(total_sweeps));
}

template <int N, class S>
void ndim_spin_sim<N, S>::save(alps::hdf5::archive & ar) const {
    mcbase::save(ar);
...
template <int N, class S>
void ndim_spin_sim<N, S>::load(alps::hdf5::archive & ar) {
    mcbase::load(ar);
...
template <int N, class S>
const typename ndim_spin_sim<N, S>::spintype ndim_spin_sim<N, S>::random_spin() {
    return spintype(random_spin_gen(random.engine()));
}

template <int N, class S>
const typename ndim_spin_sim<N, S>::spintype ndim_spin_sim<N, S>::random_spin_accept_reject() {
    double r = 0;
    typename ndim_spin_sim<N, S>::spintype spin;
