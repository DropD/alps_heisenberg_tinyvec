...
#ifndef TINYVECTOR_OE4_HPP
#define TINYVECTOR_OE4_HPP
...
class tv_addinplace_oe4 {
    public:
        static inline const tinyvector<N, OE4> & apply(tinyvector<N, OE4> &left, const tinyvector<N, OE4> &right);
};

template <int N>
class tv_addinplace_oe4<N, one> {
    public:
        static inline const tinyvector<N, OE4> & apply(tinyvector<N, OE4> &left, const tinyvector<N, OE4> &right) {
            for(int i = 0; i < N; ++i)
                left[i] += right[i];
...
template <int N> 
class tv_addinplace_oe4<N, four> {
    public:
        static inline const tinyvector<N, OE4> & apply(tinyvector<N, OE4> &left, const tinyvector<N, OE4> &right) {
            for(int i = 0; i < N/4 - 1; ++i) {
                left[4*i]       += right[4*i];
                left[4*i + 1]   += right[4*i + 1];
                left[4*i + 2]   += right[4*i + 2];
                left[4*i + 3]   += right[4*i + 3];
            }
            return left;
        }
};
...
template <int N>
inline const tinyvector<N, OE4> & operator+=(tinyvector<N, OE4> &left, const tinyvector<N, OE4> &right) {
    return tv_addinplace_oe4<N, typename multiple_of<N>::divisor>::apply(left, right);
}
//~ 
//~ template <int N>
