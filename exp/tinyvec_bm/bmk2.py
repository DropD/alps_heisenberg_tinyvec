...
class commandline_interface(object):
    def __init__(self):
        ...
        self.sp_run.add_argument('benchprog', help='the path to the benchmark executable')
...
def runbm(args):
    ...
    '''
    call = [args.benchprog, args.n, 'bm_tinyvec.dat']
...
def analyze(args):
    ...
    data = dio.read('bm_tinyvec.dat')
    x = data[:,0]
    opd = {ops[i-1] : data[:, range(i, 21, 4)] for i in range(1,5)}
    for op in ops:
        y = opd[op]
        #~ plot_runtime(x, y, op)
        plot_speedup(x, y, op)
